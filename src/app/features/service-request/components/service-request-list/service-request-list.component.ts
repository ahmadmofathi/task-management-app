...
export class ServiceRequestListComponent  implements OnInit {
  ...
  allColumns: ColumnDef[] = [
    ...
    { key: 'edit', label: 'الاجراء', selected: true },
  ];
  ...
  constructor(
    ...
    private serviceReqquestService: ServiceRequestService
  ) {
    this.searchSubject
      .pipe(
        debounceTime(600),
  ...
  openDialog() {
    const dialogRef = this.dialog.open(ServiceRequestInfoComponent, {
      width: '50vw',
  ...
  getList() {
    this.serviceReqquestService.getList().subscribe((res: any) => {
      console.log(res);
